...
class ParallelRoutePlannerTest {
    ...
    // Shared service instance — intentionally shared across all test repetitions/threads to
    // verify that PlanRouteService tolerates concurrent invocations.
    private static final PlanRouteService SERVICE =
            new PlanRouteService(new SimpleMeterRegistry(), ObservationRegistry.NOOP);

    // ── JUnit 5 native parallel: @RepeatedTest runs repetitions concurrently ──
    ...
    // ── Internal concurrency: 40 threads simultaneously planning routes ────────
    ...
    // ── Mixed ship-class parallel stress test ────────────────────────────────
    ...
    // ── Counter atomicity under concurrent access ─────────────────────────────
