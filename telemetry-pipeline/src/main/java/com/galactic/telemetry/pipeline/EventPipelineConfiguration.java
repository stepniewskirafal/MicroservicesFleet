...
public class EventPipelineConfiguration {
    ...
    @Bean
    public Function<ReservationCreatedEvent, EnrichedReservationEvent> reservationPipeline(MeterRegistry meterRegistry) {
        ...
        return event -> {
            ...
            if (event.startAt() != null && event.endAt() != null) {
                durationHours =
                        Duration.between(event.startAt(), event.endAt()).toHours();
            }
