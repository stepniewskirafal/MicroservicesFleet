...
class GlobalExceptionHandlerTest {
    ...
    void should_return_404_for_starport_not_found() {
        ResponseEntity<Map<String, String>> response = handler.handle(new StarportNotFoundException("SP-X"));
    ...
    void should_return_404_for_customer_not_found() {
        ResponseEntity<Map<String, String>> response = handler.handle(new CustomerNotFoundException("CUST-X"));
    ...
    void should_return_404_for_ship_not_found() {
        ResponseEntity<Map<String, String>> response = handler.handle(new ShipNotFoundException("SHIP-X"));
    ...
    void should_return_409_for_no_docking_bays() {
        ResponseEntity<Map<String, String>> response = handler.handle(new NoDockingBaysAvailableException(
                "SP-X", "SCOUT", Instant.now(), Instant.now().plusSeconds(3600)));
    ...
    void should_return_422_for_invalid_reservation_time() {
        ResponseEntity<Map<String, String>> response = handler.handle(new InvalidReservationTimeException(
    ...
    void should_return_409_for_route_unavailable() {
        ResponseEntity<Map<String, String>> response = handler.handle(new RouteUnavailableException("SP-A", "SP-B"));
    ...
    void should_return_500_safely_when_runtime_exception_has_null_message() {
        ResponseEntity<Map<String, String>> response = handler.handleRuntime(new RuntimeException());
